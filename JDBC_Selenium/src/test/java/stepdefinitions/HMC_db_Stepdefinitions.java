...
public class HMC_db_Stepdefinitions {
    // url,username, pasword ve tablo isimleri bize kesinlikle verilmeli
    ...
    ResultSet resultSet;    // Query sonucunda dönen sonucu store etmemize yariyor

    @Given("kullanici HMC veri tabanina baglanir")
    public void kullanici_hmc_veri_tabanina_baglanir() throws SQLException {
        connection= DriverManager.getConnection(url,username,password);
        // Aşağıdaki Satır aynen olmalı veya bilgiyi bize verilmeli
    ...
    @And("kullanici {string} tablosundaki tüm verileri alir")
    public void kullaniciTablosundakiTümVerileriAlir(String table) throws SQLException {
        String query = "SELECT * FROM "+ table; // Sorguya ne yazarsak o çalışır.
        resultSet = statement.executeQuery(query);
    }

    @And("kullanici verileri okur")
    public void kullaniciVerileriOkur() throws SQLException {
        while (resultSet.next()){
            System.out.printf("%-6d %-6s %-20s %-20s %-20s\n",resultSet.getInt(1),resultSet.getString(2),
